...
class MIMBlock(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, configs):
        r"""
        ...
        self.forget_bias = 0.01


        self.mim_n = MIMN(in_channels=in_channels, hidden_channels=hidden_channels, kernel_size=self.kernel_size,
                          forget_bias=self.forget_bias)

        self.mim_s = MIMS(hidden_channels=hidden_channels, kernel_size=self.kernel_size, forget_bias=self.forget_bias)

        self.conv_x = nn.Conv2d(
            in_channels=in_channels, out_channels=hidden_channels * 6,
            kernel_size=self.kernel_size, padding=self.padding, stride=(1, 1)
        )
    ...
    def forward(self, x_, x, h, states):
        r"""
        :param x_:   h_{t-1}^{l-1}
        :param x:    h_{t-1}^l
        :param c:    cell记忆信息
...
class MIMN(nn.Module):
    ...
    def __init__(self, in_channels: int, hidden_channels: int, kernel_size: Union[int, tuple],
        ...
        self.conv_h_diff = nn.Conv2d(
            ...
            kernel_size=kernel_size, padding=self.padding, stride=(1, 1)
        )
